...
def create_quarterly_data(use_japanese_columns):
    ...
    for ticker, name in tickers.items():
        ...
        try:
            ...
            try:
                # インデックスがDatetimeIndexでない場合、変換を試みる
    ...
    for ticker, name in tickers.items():
        ...
        for retry in range(max_retries):
            try:
                ...
                # 既存のエクセルファイルのシートを保持しつつ、新しいシートを追加
                with pd.ExcelWriter(temp_excel_path, engine='openpyxl') as writer:
                    # まず既存のシートを読み込んで一時ファイルに書き込む
                    try:
                        xls = pd.ExcelFile(excel_path)
                        existing_sheets = xls.sheet_names
                        
                        # 既存のシートをコピー（四半期足と年足以外）
                        ...
                        print(f"既存のシート読み込みエラー: {e}")
                        # エラーでも続行し、新しいシートだけを作成
                ...
                if os.path.exists(temp_excel_path):
                    try:
                        ...
                        print(f"ファイル置換エラー: {e}")
                        if os.path.exists(temp_excel_path):
                            print(f"一時ファイルは保存されています: {temp_excel_path}")
                ...
                if retry < max_retries - 1:
                    ...
                    print(f"5秒後にリトライします... ({retry+1}/{max_retries})")
                    time.sleep(5)  # 5秒待って再試行
                else:
                    print(f"最大リトライ回数を超えました。ファイルへのアクセスができません: {e}")
                    # 一時ファイルを保存
                    temp_excel_path = os.path.join(output_dir, f"{safe_ticker}_{name}_backup.xlsx")
                    try:
                        if quarterly_data is not None or yearly_data is not None:
                            with pd.ExcelWriter(temp_excel_path, engine='openpyxl') as writer:
                                if quarterly_data is not None:
                                    quarterly_data.to_excel(writer, sheet_name="四半期足")
                                if yearly_data is not None:
                                    yearly_data.to_excel(writer, sheet_name="年足")
                            print(f"バックアップファイルを保存しました: {temp_excel_path}")
                    except Exception as backup_error:
                        print(f"バックアップファイル作成エラー: {backup_error}")
