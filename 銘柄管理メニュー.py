    ...
    def rename_portfolio(self):
        ...
        if new_name and new_name.strip():
            ...
            combo = ttk.Combobox(values=portfolio_names)
    ...
    def add_new_portfolio(self):
        """新しいポートフォリオを追加する"""
        # 最大数のチェック
        if len(self.config['portfolios']) >= 10:
            messagebox.showerror("エラー", "ポートフォリオは最大10個までしか作成できません。")
            return
        ...
        if new_name and new_name.strip():
            ...
            for portfolio in self.config['portfolios']:
                if portfolio['name'] == new_name.strip():
                    messagebox.showerror("エラー", f"ポートフォリオ名 '{new_name}' はすでに使用されています。")
                    return
            ...
            messagebox.showinfo("成功", f"新しいポートフォリオ '{new_name.strip()}' を作成しました。")
    ...
    def update_portfolio_settings_ui(self):
        ...
        # ポートフォリオ名のリスト表示を更新
        for i, portfolio in enumerate(self.config['portfolios']):
            frame = ttk.Frame(portfolio_list_frame)
    ...
    def reset_portfolio_name_entry(self, index, original_name):
        ...
        if not portfolio_list_frame and index < len(portfolio_list_frame.winfo_children()):
            return
        
        try:
            frame = portfolio_list_frame.winfo_children()[index]
...
class StockConfigApp:
    ...
    def setup_stocks_tab(self):
        ...
        copy_to_portfolio_button = ttk.Button(copy_frame, text="ポートフォリオへコピー", 
                                              command=lambda: self.copy_to_portfolio(self.stock_list))
    ...
    def setup_favorites_tab(self):
        ...
        copy_to_portfolio_button = ttk.Button(copy_frame, text="ポートフォリオへコピー", 
                                             command=lambda: self.copy_to_portfolio(self.favorites_list))
        copy_to_portfolio_button.pack(fill=tk.X, pady=5)
    ...
    def setup_settings_tab(self):
        ...
        # ポートフォリオ設定
        portfolio_frame = ttk.LabelFrame(settings_frame, text="ポートフォリオ設定", padding="10")
