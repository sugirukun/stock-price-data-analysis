...
class StockConfigApp:
    def __init__(self, root):
        ...
        # ウィンドウ終了時の処理をカスタム処理に変更
        self.root.protocol("WM_DELETE_WINDOW", self.on_close)
        
        # 設定の読み込み
        self.config = load_config()
        
        # 元の設定をディープコピーで保存（変更検出用）
    ...
    def setup_settings_tab(self):
        ...
        # この参照を保存しておく
        self.portfolio_settings_frame = portfolio_frame
        ...
        # これらの参照も保存しておく
        self.portfolio_list_frame = portfolio_list_frame
        self.portfolio_canvas = canvas
        self.portfolio_scrollable_frame = scrollable_frame
        ...
        for i, portfolio in enumerate(self.config['portfolios']):
            ...
            up_button = ttk.Button(button_frame, text="↑", width=2, 
                                  command=lambda idx=i: self.move_portfolio_up(idx))
            up_button.pack(side=tk.LEFT, padx=2)
            
            down_button = ttk.Button(button_frame, text="↓", width=2,
                                    command=lambda idx=i: self.move_portfolio_down(idx))
    ...
    def update_portfolio_settings_ui(self):
        ...
        # 保存した参照を使用する
        if not hasattr(self, 'portfolio_scrollable_frame') or not hasattr(self, 'portfolio_canvas'):
            print("警告: ポートフォリオ設定UIの参照が見つかりません")
            return
        
        scrollable_frame = self.portfolio_scrollable_frame
    ...
    def move_portfolio_up(self, index):
        """ポートフォリオを上に移動する"""
        if index <= 0 or index >= len(self.config['portfolios']):
            return  # 先頭かリスト外の場合は何もしない
        
        # ポートフォリオの交換
        portfolios = self.config['portfolios']
        portfolios[index], portfolios[index-1] = portfolios[index-1], portfolios[index]
        
        # 現在表示中のポートフォリオインデックスを更新
        if self.current_portfolio_index == index:
            self.current_portfolio_index = index - 1
        elif self.current_portfolio_index == index - 1:
            self.current_portfolio_index = index
        
        # 設定変更フラグをセット
        self.config_changed = True
        
        # ポートフォリオタブのコンボボックスを更新
        self.update_portfolio_combo()
        
        # 一般設定タブの表示を更新
        self.update_portfolio_settings_ui()
    
    def move_portfolio_down(self, index):
        """ポートフォリオを下に移動する"""
        if index < 0 or index >= len(self.config['portfolios']) - 1:
            return  # 末尾かリスト外の場合は何もしない
    ...
    def update_portfolio_combo(self):
        ...
        if combo_widget:
            ...
            if 0 <= self.current_portfolio_index < len(self.config['portfolios']):
                current_name = self.config['portfolios'][self.current_portfolio_index]['name']
                combo_widget[0].set(current_name)
    ...
    def reset_portfolio_name_entry(self, index, original_name):
        ...
        if not hasattr(self, 'portfolio_scrollable_frame'):
            return
    ...
    def move_items(self, tree, selected_items, target_item):
        ...
        # 順序が変更されたら設定変更フラグをセット
        if target_list != target_list_new:
            self.config_changed = True
    ...
    def on_close(self):
        ...
        response = messagebox.askquestion(
            "冒険の書",
            "では　神のまえにこれまでの　おこないを　告白なさい。\nそして　この冒険の書に　記録しても　よろしいかな？",
            type=messagebox.YESNO,
            icon=messagebox.QUESTION,
            default=messagebox.YES
        )
        
        # メッセージボックスのボタンテキストを「うん」「いやだ」に変更
        for button in self.root.winfo_children():
            if isinstance(button, tk.Toplevel):
        ...
        if response == "yes":  # 「うん」が選択された場合
            self.confirm_save_quit()
        else:  # 「いやだ」が選択された場合
            self.confirm_quit_without_save()
    
    def confirm_save_quit(self):
        """保存して終了する確認処理"""
        # 設定を保存
    ...
    def confirm_quit_without_save(self):
        ...
        # メッセージボックスのボタンテキストを「つづける」「やめる」に変更
        for button in self.root.winfo_children():
            if isinstance(button, tk.Toplevel):
                for child in button.winfo_children():
                    if isinstance(child, ttk.Frame):
        ...
        if response == "yes":  # 「つづける」が選択された場合
            # 何もしない（アプリを継続）
            return
        else:  # 「やめる」が選択された場合
            self.confirm_quit_no_save_final()
    
    def confirm_quit_no_save_final(self):
        """保存せずに終了する最終確認"""
        response = messagebox.askquestion(
            "冒険の書",
            "なんと　記録せずに　休むと　申されるか……。\nでは　そなたの冒険が　前の記録まで　もどっても　よろしいのじゃな？",
            type=messagebox.YESNO,
            icon=messagebox.QUESTION
        )
        
        if response == "yes":  # 「はい」が選択された場合
            # 終了メッセージ
            messagebox.showinfo(
                "休息",
                "おお　神よ！　この者たちに　ひとときの　休息を　あたえたまえ！"
            )
